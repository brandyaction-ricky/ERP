...
export function PayrollManagement({ payrolls, employees, contracts, saving, onSave, onDelete }: { payrolls: Payroll[]; employees: Employee[]; contracts: Contract[]; saving: boolean; onSave: Save; onDelete: (id: number) => void }) {
  ...
  const calculated = calculateInsurance(form.payrollMonth, number(form.pensionBase), number(form.healthBase), number(form.employmentBase));
  ...
  const deductionTotal = insurance.nationalPension + insurance.healthInsurance + insurance.longTermCare + insurance.employmentInsurance + number(form.incomeTax) + number(form.localIncomeTax) + number(form.otherDeduction);
  ...
  async function submit(event: FormEvent) {
    ...
    const success = await onSave({ action: "payroll", ...form }, form.id ? "PATCH" : "POST");
  ...
  return <>
    ...
    <section className="workspace">
      <div className="section-head"><div><h2>{month.replace("-", "년 ")}월 급여대장</h2><p>직원별 지급액·공제액·차인지급액을 관리합니다.</p></div><div className="filters"><input type="month" value={month} onChange={(event) => setMonth(event.target.value)} /><button className="primary-button compact" onClick={() => open()}>+ 급여 등록</button></div></div>
    ...
    {modal && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setModal(false); }}><section className="modal wide payroll-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><h2>{form.id ? "급여대장 수정" : "급여대장 등록"}</h2><p>지급 항목과 기준 보수월액을 입력하면 4대보험 근로자 부담분을 계산합니다.</p></div><button onClick={() => setModal(false)} aria-label="닫기">×</button></div><form className="modal-form" onSubmit={submit}>
      ...
      <div className="payroll-form-section"><div className="payroll-section-head"><div><h3>4대보험 자동계산</h3><p>2026년 근로자 부담 요율 기준이며 실제 고지액과 차이가 날 수 있습니다.</p></div><label className="toggle-label"><input type="checkbox" checked={form.autoInsurance} onChange={(event) => setForm({ ...form, autoInsurance: event.target.checked })} /> 자동계산</label></div><div className="form-grid three"><label className="field"><span>국민연금 기준소득월액</span><input type="number" min="0" value={form.pensionBase} onChange={(event) => setForm({ ...form, pensionBase: event.target.value })} /></label><label className="field"><span>건강보험 보수월액</span><input type="number" min="0" value={form.healthBase} onChange={(event) => setForm({ ...form, healthBase: event.target.value })} /></label><label className="field"><span>고용보험 보수월액</span><input type="number" min="0" value={form.employmentBase} onChange={(event) => setForm({ ...form, employmentBase: event.target.value })} /></label></div><div className="insurance-preview">{[["국민연금 4.75%", insurance.nationalPension], ["건강보험 3.595%", insurance.healthInsurance], ["장기요양", insurance.longTermCare], ["고용보험 0.9%", insurance.employmentInsurance]].map(([label, value]) => <div key={String(label)}><span>{label}</span><strong>{money(Number(value))}</strong></div>)}</div>{!form.autoInsurance && <div className="form-grid four">{[["국민연금", "nationalPension"], ["건강보험", "healthInsurance"], ["장기요양", "longTermCare"], ["고용보험", "employmentInsurance"]].map(([label, key]) => <label className="field" key={key}><span>{label}</span><input type="number" min="0" value={form[key as keyof typeof form] as string} onChange={(event) => setForm({ ...form, [key]: event.target.value })} /></label>)}</div>}</div>
      <div className="payroll-form-section"><h3>세금·기타 공제</h3><div className="form-grid three"><label className="field"><span>소득세</span><input type="number" min="0" value={form.incomeTax} onChange={(event) => setForm({ ...form, incomeTax: event.target.value, localIncomeTax: String(Math.floor(number(event.target.value) * 0.1 / 10) * 10) })} /></label><label className="field"><span>지방소득세</span><input type="number" min="0" value={form.localIncomeTax} onChange={(event) => setForm({ ...form, localIncomeTax: event.target.value })} /></label><label className="field"><span>기타 공제</span><input type="number" min="0" value={form.otherDeduction} onChange={(event) => setForm({ ...form, otherDeduction: event.target.value })} /></label></div><div className="payroll-result"><div><span>공제 합계</span><strong>{money(deductionTotal)}</strong></div><div><span>차인지급액</span><strong>{money(Math.max(0, payTotal - deductionTotal))}</strong></div></div></div>
      ...
      <div className="payroll-rate-note"><strong>2026 적용 기준</strong><span>국민연금 9.5%(근로자 4.75%) · 건강보험 7.19%(근로자 3.595%) · 장기요양 0.9448% · 고용보험 근로자 0.9%</span><small>소득세는 부양가족 수 등에 따라 달라지므로 국세청 간이세액표의 금액을 입력합니다.</small></div>
